...
class LocalEncoder(nn.Module):
    def __init__(self, historical_steps: int, node_dim: int, edge_dim: int, embed_dim: int,
                 num_heads: int = 8, dropout: float = 0.1, num_temporal_layers: int = 4,
                 local_radius: float = 50, parallel: bool = False) -> None:
        super().__init__()
        ...
        self.temporal_encoder = TemporalEncoder(historical_steps, embed_dim, num_heads, num_temporal_layers, dropout)
        self.al_encoder = ALEncoder(node_dim, edge_dim, embed_dim, num_heads, dropout)

    def forward(self, data: TemporalData) -> torch.Tensor:
        # Precompute edge indices and attributes for all timesteps
        for t in range(self.historical_steps):
            data[f'edge_index_{t}'], _ = subgraph(~data['padding_mask'][:, t], edge_index=data.edge_index)
            data[f'edge_attr_{t}'] = data['positions'][data[f'edge_index_{t}'][0], t] - \
                                      data['positions'][data[f'edge_index_{t}'][1], t]

        # Encode node features in parallel or sequentially
        if self.parallel:
            snapshots = []
            for t in range(self.historical_steps):
                edge_index, edge_attr = self.drop_edge(data[f'edge_index_{t}'], data[f'edge_attr_{t}'])
                snapshots.append(Data(x=data.x[:, t], edge_index=edge_index, edge_attr=edge_attr, num_nodes=data.num_nodes))
            batch = Batch.from_data_list(snapshots)
            out = self.aa_encoder(x=batch.x, t=None, edge_index=batch.edge_index,
                                  edge_attr=batch.edge_attr, bos_mask=data['bos_mask'], rotate_mat=data['rotate_mat'])
            out = out.view(self.historical_steps, out.shape[0] // self.historical_steps, -1)
        else:
            out = []
            for t in range(self.historical_steps):
                edge_index, edge_attr = self.drop_edge(data[f'edge_index_{t}'], data[f'edge_attr_{t}'])
                out.append(self.aa_encoder(x=data.x[:, t], t=t, edge_index=edge_index,
                                           edge_attr=edge_attr, bos_mask=data['bos_mask'][:, t],
                                           rotate_mat=data['rotate_mat']))
            out = torch.stack(out)  # [T, N, D]

        # Temporal encoding
        out = self.temporal_encoder(out, padding_mask=data['padding_mask'][:, :self.historical_steps])
...
class AAEncoder(MessagePassing):
    def __init__(self, historical_steps: int, node_dim: int, edge_dim: int, embed_dim: int,
                 num_heads: int = 8, dropout: float = 0.1, parallel: bool = False, **kwargs) -> None:
        ...
        self.center_embed = SingleInputEmbedding(node_dim, embed_dim)
        self.nbr_embed = MultipleInputEmbedding([node_dim, edge_dim], embed_dim)
        ...
        self.mlp = nn.Sequential(nn.Linear(embed_dim, 4*embed_dim), nn.ReLU(), nn.Dropout(dropout),
    ...
    def forward(self, x: torch.Tensor, t: Optional[int], edge_index: Adj,
                edge_attr: torch.Tensor, bos_mask: torch.Tensor,
                rotate_mat: Optional[torch.Tensor] = None, size: Size = None) -> torch.Tensor:
        if self.parallel:
            ...
            center_embed = torch.where(bos_mask.t().unsqueeze(-1), self.bos_token.unsqueeze(-2), center_embed)
            center_embed = center_embed.view(x.shape[0], -1)
        else:
            if rotate_mat is None:
    ...
    def message(self, edge_index: Adj, center_embed_i: torch.Tensor, x_j: torch.Tensor,
        ...
        if rotate_mat is None:
            ...
            center_rotate_mat = rotate_mat.repeat(self.historical_steps, 1, 1) if self.parallel else rotate_mat
            center_rotate_mat = center_rotate_mat[edge_index[1]]
        ...
        return value * self.attn_drop(alpha.unsqueeze(-1))
    ...
    def _mha_block(self, center_embed, x, edge_index, edge_attr, rotate_mat, size):
...
# TemporalEncoder and ALEncoder follow the same pattern as before, with safe rotate_mat handling,
